...
def validate(model, prefix, draft_model_output, temperature, top_k, top_p):
    tmp_prefix = torch.concat([prefix, draft_model_output[:, 1:]], dim=1)
    ...
    # end_time = time.time()
    # print(f"\n=========================================VALIDATION DATABASE=========================================")
    # print(f"Time taken: {end_time - start_time}, Tokens validated: {draft_model_output.shape[1]}, Tokens per second: {(draft_model_output.shape[1]) / (end_time - start_time)}\n")
